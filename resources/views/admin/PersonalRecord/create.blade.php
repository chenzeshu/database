...
@section('content')
    ...
    <div class="result_wrap">
        <form action="{{url('admin/PR/')}}" method="post" enctype="multipart/form-data">
            {{csrf_field()}}
            <table class="add_tab">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <select name="fileid" id="">
                                <option value="1" @if(session('fileid')==1) selected @endif>个人证件照</option>
                                <option value="2" @if(session('fileid')==2) selected @endif>身份证</option>
                                <option value="3" @if(session('fileid')==3) selected @endif>学历学位</option>
                                <option value="4" @if(session('fileid')==4) selected @endif>职称证书</option>
                                <option value="5" @if(session('fileid')==5) selected @endif>培训证书</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <th>姓名：</th>
                        <td>
                            <input type="text" class="xs" name="name" value="">
