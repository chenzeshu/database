            ...
            <div class="result_content">
                <table class="list_tab">
                    ...
                    <tr>
                        <td class="tc">{{$num++}}</td>
                        <td>
                            <a href="{{url('admin/JC/index').'/'.$v->id}}">{{$v['profession']}}</a>
                        </td>
                        <td>
                            {{$v->updated_at}}
                        </td>
                        <td>
                            <a href="{{url('admin/JCP/'.$v->id.'/edit')}}">修改</a>
    ...
    <script>
        ...
        function deleteObj(id){
            layer.confirm('确定删除这位仁兄？', {
                ...
                $.post("{{url('admin/JCP/')}}/"+id,{'_method':'delete','_token':"{{csrf_token()}}"},function(data){
                    if(data.status=='0'){
        ...
        function searchName(){
            ...
            $.post("{{url('admin/kao/search')}}",{name:val,'_token':'{{csrf_token()}}'},function(data){
                ...
                $(data).each(function(k,v){
                    ...
                    var url2 = 'javascrJCPt:onclick=deleteObj('+v.kao_id+')';
                    if(v.kao_if==1){
                        v.kao_if="提交";
                    }else {
                        v.kao_if="未提交";
