...
@section('content')
    ...
    <div class="result_wrap">
        <form action="{{url('admin/PRO/'.$data->id)}}" method="post">
            {{method_field('PUT')}}
            {{csrf_field()}}
            <table class="add_tab">
                <tbody>
                <tr>
                    ...
                    <td>
                        <select name="fileid" id="">
                            <option value="1" @if($data['fileid']==1) selected @endif>中标通知书</option>
                            <option value="2" @if($data['fileid']==2) selected @endif>验收报告</option>
                            <option value="3" @if($data['fileid']==3) selected @endif>感谢信</option>
                            <option value="4" @if($data['fileid']==4) selected @endif>合同</option>
                            <option value="5" @if($data['fileid']==5) selected @endif>投标文件</option>
                            <option value="6" @if($data['fileid']==6) selected @endif>招标文件</option>
                            <option value="7" @if($data['fileid']==7) selected @endif>变更资料</option>
                            <option value="8" @if($data['fileid']==8) selected @endif>资质</option>
                        </select>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th>行业</th>
                    <td>
                        <input type="text" class="xs" name="profession" value="{{$data->profession}}">
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th>项目经理</th>
                    <td>
                        <input type="text" class="xs" name="PM" value="{{$data->PM}}">
                    </td>
                </tr>
                <tr>
                    <th>金额</th>
                    <td>
                        <input type="text" class="xs" name="sum" value="{{$data->sum}}">
                    </td>
                </tr>
                <tr id="valid">
                    <th>合同生效时间</th>
                    <td>
                        <input placeholder="请输入日期" class="laydate-icon"
                               onclick="laydate()" name="valid_time" value="{{date('Y-m-d',$data->valid_time)}}">
                    </td>
                </tr>
                <tr>
                    <th>上传文件</th>
